...
As a best practice, the RC may send the list of proposed rules changes to District Coordinators ahead of the 3-month final review period.

## Annual CQLT Ratification

Annually, the CQLT will review each individual issue (PR) with voted-upon changes by the RC. A simple majority of the CQLT is required to ratify each individual issue (PR). Each ratified PR will be merged to the master file immediately following that season's IBQ. Whatever is in the master file will be considered the rule book and supporting documentation for the upcoming IBQ.

The CQLT, at its discretion, may choose to vote to ratify or reject the entire block of rules changes as a whole. Unanimous consent by the CQLT would be required to put the entire block of rules change as a whole up for a vote. If unanimous consent is achieved, then a simple majority would be required to ratify or reject the block of rules changes as a whole.

Any RC members who are also on the CQLT must cast their vote in favor of all rules changes that the RC is proposing to the CQLT.
